...
if [ "$TAR_EXIT" -eq 0 ] || [ "$TAR_EXIT" -eq 1 ]; then
    ...
    log_ok "Backup concluído! Tamanho: $(du -sh "$ARQUIVO_FINAL" | awk '{print $1}')"
else
    die "Erro ao criar arquivo de backup (código tar: $TAR_EXIT)."
fi

# 3. Rotação de Backups Antigos (específica por perfil)
log_info "Verificando retenção (removendo arquivos com mais de ${RETENCAO_DIAS} dias no perfil ${NOME_PERFIL})..."
find "$DESTINO_FINAL" -maxdepth 1 -name "backup_${NOME_PERFIL}_*.tar.gz" -type f -mtime "+${RETENCAO_DIAS}" -print -delete
